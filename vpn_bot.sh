...
cat > /usr/local/bin/vpn-user-manager << 'SCRIPTEOF'
#!/bin/bash
...
cat > /usr/local/bin/generate-openclash.py << 'PYEOF'
#!/usr/bin/env python3
...
cat > /opt/vpn-bot/vpn-bot.py << BOTEOF
#!/usr/bin/env python3
import logging
import sqlite3
import datetime
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [InlineKeyboardButton("🛍️ Beli VPN", callback_data="buy_vpn")],
...
async def check_balance(query, context):
    ...
    await query.edit_message_text(f"💰 **Saldo Anda:** Rp {balance:,}", parse_mode='Markdown')
...
2. 🛍️ **Beli VPN** - Pilih server dan buat akun
...
delete_user() {
    echo -e "${YELLOW}🗑️ Delete VPN User:${NC}"
